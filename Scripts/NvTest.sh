    ...
    if [[ -z $2 ]]; then 
        ...
        function wzhu-rsync-to-windows {
            if [[ ! -z $1 ]]; then 
                ...
                sshpass -p "$(echo 'U2FsdGVkX1+UnE9oAYZ8DjyHzGqQ3wxZbhrJanHFw9u7ypNWEkG2dOJQShrj5dlT' | openssl enc -d -aes-256-cbc -pbkdf2 -a)" scp -r -v -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null $@ WanliZhu@$host:'C:\Users\WanliZhu\Desktop\'
            fi 
        }
    fi 
