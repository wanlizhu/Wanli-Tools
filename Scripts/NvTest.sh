...
if [[ $1 == driver || $1 == drivers ]]; then 
    ...
    if [[ $WZHU_PI == 1 ]]; then 
        ...
        if [[ ! -z $2 ]]; then 
            if [[ $WZHU_GDB == 1 ]]; then 
                commandLine="$GL_ENV cd $WZHU_VP_ROOT && gdb -ex 'set trace-commands on' -ex 'set pagination off' -ex 'set confirm off' -ex 'set debuginfod enabled on' -ex 'set breakpoint pending on' -ex 'file ./viewperf/bin/viewperf' -ex \"set args viewsets/$2/config/$2.xml $3 -resolution 3840x2160\""
            else
